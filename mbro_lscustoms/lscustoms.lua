...
local garages = {
	...
	[16] = { locked = false, outside = {x = -919.51348876953, y = -1455.3988037109, z = 0.051255792379379, heading = 289.67205810547}, inside = {x = -919.51348876953, y = -1455.3988037109, z = 0.051255792379379, heading = 109.67205810547}}, -- LC Shoreside Vale
    [17] = { locked = false, outside = {x = 635.80340576172, y = 254.47659301758, z = 102.57917785645, heading = 289.67205810547}, inside = {x = 636.32067871094, y = 255.50833129883, z = 102.56970214844, heading = 75.335639953613}}, -- Downtown Vinewood

    --arena
    [18] = { hidden = true, locked = false, outside = {x = 2844.9555664063, y = -3912.4829101563, z = 140.00074768066, heading = 0.0}, inside = {x = 2844.9555664063, y = -3912.4829101563, z = 140.00074768066, heading = 0.0}, free = true}, -- Arena
	[19] = { hidden = true, locked = false, outside = {x = 2755.0791015625, y = -3687.3322753906, z = 140.00091552734, heading = 180.0}, inside = {x = 2755.0791015625, y = -3687.3322753906, z = 140.00091552734, heading = 180.0}, free = true}, -- Arena
}
...
function OverrideCurrentPosition()
    local pos = GetEntityCoords(PlayerPedId())
    local heading = GetEntityHeading(PlayerPedId())
    OverridePosition(pos.x, pos.y, pos.z, heading)
end

function OverridePosition(x, y, z, h)
    currentpos = {
...
RegisterNetEvent("omni:lsc:force")
AddEventHandler("omni:lsc:force", function()
    if IsPedInAnyVehicle(PlayerPedId(), false) and not inside then
        ForceModShop()
    end
end)
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		myveh.paintjob = GetVehicleLivery(veh)
		myveh.extras = {}
		myveh.mods = {}
		for i = 0, 48 do
		...
		respray = LSCMenu.categories:addSubMenu("RESPRAY", "Respray & Paintjobs", "Transforms vehicle appearance.",true)

            AddMod(48,respray,"Liveries", "Liveries", "A selection of decals for your vehicle.",true)
            if GetVehicleLiveryCount(veh) > 0 then
                m = respray:addSubMenu("PAINTJOBS", "Paintjobs", "Special liveries and paintjobs",true)
                    for livery = 0, GetVehicleLiveryCount(veh) - 1 do
                        local liveryn = (livery + 1)
                        local paintjobLabel = vehicleName .. "_S" .. liveryn
                        local paintjobText = GetLabelText(paintjobLabel)
                        btn = m:addPurchase(paintjobText ~= "NULL" and paintjobText or "Paintjob #" .. liveryn, livery * 500) btn.paintjob = livery
                    end
            end
        ...
        if numExtras > 0 then
		    extraMenu = LSCMenu.categories:addSubMenu("EXTRA PARTS", "Extra Parts", "Misc. parts that can be toggled",true)
            for _, extraId in next, extras do
                btn = extraMenu:addPurchase("Extra Part #" .. (extraId), 0) btn.extra = extraId
            end
        end
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		elseif IsThisModelABoat(model) then
			vtype = "boat"
		elseif exports['omni_common']:IsModelATrailer(model) then
			vtype = "trailer"
		elseif exports['omni_common']:IsModelASemiCab(model) then
			vtype = "cab"
		else
			vtype = "car"
		end
		...
		for i = 0, 48 do
            ...
            if DoesExtraExist(veh, i) then
                mods.allmods["e" .. i] = IsVehicleExtraTurnedOn(veh, i)
            end
		end
        local livery = GetVehicleLivery(veh)
        if livery ~= -1 then
            mods.allmods["livery"] = livery
        end
		...
		TriggerServerEvent("LSC:finished", vehicle, mods)
...
Citizen.CreateThread(function()
	while true do
		...
		if inside == false then
            ...
            local isInVehicle, veh = IsPedInAnyVehicle(ped, false), GetVehiclePedIsIn(ped)
            if isInVehicle and DecorExistOn(veh, "omni_norepmod") and DecorGetBool(veh, "omni_norepmod") then
                isInVehicle = false
            end
			...
			if isInVehicle then
				...
				if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped then
					...
					for i,pos in ipairs(garages) do
						...
						outside = pos.outside
                        local dist = #(vec3(outside.x, outside.y, outside.z) - pedPos)
						...
						if LSC_Config.oldenter then
                            ...
                            if dist <= 50.0 then
                                DrawMarker(1, outside.x, outside.y, outside.z - 2.0, 0,0,0,0,0,0,5.0,5.0,2.0,255,255,255,200,0,0,0,0)
    							if dist <= 10.0 then
							...
							if math.abs(GetEntityHeading(veh)-outside.heading) <= 90 and IsVehicleStopped(veh) and dist <= f(5) then
								--Lets check if our vehicle is not in the model black list, and if it is not then we can go further
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									--If the garage is locked
									if pos.locked then
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	elseif m == "paintjobs" then
		SetVehicleLivery(veh, button.paintjob)
	else
	end
	if m == "horn" then
...
function LSCMenu:onButtonSelected(name, button)
	...
	TriggerServerEvent("LSC:buttonSelected", name, button, currentpos.free)
end
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	elseif mname == "paintjobs" then
		if button.purchased or CanPurchase(price, canpurchase) then
			myveh.paintjob = button.paintjob
			SetVehicleLivery(veh,button.paintjob)
		end
	elseif mname == "extra parts" then
		if button.purchased or CanPurchase(price, canpurchase) then
            local wasEnabled = not IsVehicleExtraTurnedOn(veh, button.extra)
			myveh.extras[button.extra] = wasEnabled
			SetVehicleExtra(veh, button.extra, not wasEnabled)
		end
...
function CheckPurchases(m)
	...
	elseif name == "paintjobs" then
		for i,b in pairs(m.buttons) do
			if myveh.paintjob == b.paintjob then
				b.sprite = "garage"
			else
				b.sprite = nil
			end
		end
	elseif name == "extra parts" then
		for i,b in pairs(m.buttons) do
			if DoesExtraExist(myveh.vehicle, b.extra) and IsVehicleExtraTurnedOn(myveh.vehicle, b.extra) then
				b.sprite = "shop_box_tick"
			else
				b.sprite = "shop_box_blank"
			end
		end
...
function UnfakeVeh()
	...
	SetVehicleLivery(veh, myveh.paintjob)
	...
	for i,m in pairs(myveh.extras) do
		SetVehicleExtra(veh, i, not m)
	end
